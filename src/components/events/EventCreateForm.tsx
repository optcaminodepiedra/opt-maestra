...
export function EventCreateForm({ data }: { data: EventCreateData }) {
  ...
  const [status, setStatus] = React.useState<EventStatusCreateValue>("DRAFT");
  const [businessId, setBusinessId] = React.useState(data.defaultBusinessId);

  const [startsAtLocal, setStartsAtLocal] = React.useState(initialStart);
  const [endsAtLocal, setEndsAtLocal] = React.useState("");
  const [locationBusinessId, setLocationBusinessId] = React.useState(data.defaultBusinessId);
  const [locationName, setLocationName] = React.useState("");
  const [locationAddress, setLocationAddress] = React.useState("");

  const [estimatedGuests, setEstimatedGuests] = React.useState("0");
  const [confirmedGuests, setConfirmedGuests] = React.useState("0");
  const [contactName, setContactName] = React.useState("");
  const [contactPhone, setContactPhone] = React.useState("");
  const [contactEmail, setContactEmail] = React.useState("");

  const [responsibleUserId, setResponsibleUserId] = React.useState(data.creator.id);
  const [description, setDescription] = React.useState("");
  const [internalNotes, setInternalNotes] = React.useState("");
  const [isPrivate, setIsPrivate] = React.useState(false);

  const [paymentTiming, setPaymentTiming] =
    React.useState<EventPaymentTimingValue>("NOT_DEFINED");
  const [paymentStatus, setPaymentStatus] =
    React.useState<EventPaymentStatusValue>("PENDING");
  const [quotedAmount, setQuotedAmount] = React.useState("0");
  const [paidAmount, setPaidAmount] = React.useState("0");
  const [paymentDueLocal, setPaymentDueLocal] = React.useState("");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/app/events?created=${result.eventId}`);
      router.refresh();
    } catch (caught) {
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Registra la información operativa, responsable, pagos, requerimientos y requisiciones.
          </p>
        </div>
        ...
        <div className="space-y-5 lg:col-span-2">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            updateRequirement(requirement.key, {
                              neededByLocal: event.target.value,
                            })
                          }
                        />
                      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Solo aparecen requisiciones activas que todavía no están vinculadas a otro evento.
              </p>
            </CardContent>
          </Card>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-lg border bg-muted/30 p-3">
                ...
                <p className="text-sm font-medium mt-1">{data.creator.fullName}</p>
                <p className="text-xs text-muted-foreground">{data.creator.role}</p>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setPaymentDueLocal(event.target.value)}
                  disabled={paymentTiming === "NO_CHARGE"}
                />
              </div>
