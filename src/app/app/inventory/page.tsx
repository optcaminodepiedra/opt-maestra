...
import {
  ...
  ClipboardList, History, Warehouse,
} from "lucide-react";
...
export default async function InventoryPage({
  ...
  const isInventoryRole = role === "INVENTORY" || PRIVATE_VIEWERS.includes(role);
  ...
  // Stock crítico SOLO en Almacén General
  ...
  const [totalItems, businessCount, urgentRequisitions] = await Promise.all([
    prisma.inventoryItem.count({
      where: { isActive: true, businessId: ALMACEN_GENERAL_ID },
    }),
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
          ...
          {isInventoryRole && (
            <>
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/inventory/stock">
                  <Warehouse className="w-4 h-4 mr-1.5" /> Stock físico
                </Link>
              </Button>
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/inventory/movements">
                  <History className="w-4 h-4 mr-1.5" /> Movimientos
                </Link>
              </Button>
            </>
          )}
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          subtitle={`Almacén general · ${businessCount} negocios`}
        ...
        <KpiCard
          ...
          subtitle={lowStockCount === 0 ? "Stock óptimo" : "Productos agotados"}
          href="/app/inventory/stock?stock=OUT"
        />
      </div>
...
function KpiCard({
  color, icon, label, value, subtitle, href,
}: {
  color: string; icon: React.ReactNode; label: string; value: string; subtitle: string; href?: string;
}) {
  const inner = (
    <Card className={`border-l-4 border-l-${color}-500 py-0 ${href ? "hover:shadow-md transition-shadow cursor-pointer" : ""}`}>
