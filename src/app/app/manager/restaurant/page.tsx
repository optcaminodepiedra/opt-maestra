...
import {
  UtensilsCrossed, DollarSign, ClipboardList, Users,
  AlertTriangle, ArrowRight, CheckSquare, TrendingUp,
  ShoppingCart, Calendar, Package, Building2, Utensils,
  TrendingDown,
} from "lucide-react";
...
export default async function RestaurantManagerDashboard() {
      ...
      <div className="p-6 max-w-xl mx-auto">
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground">
            Tu usuario no tiene un negocio principal. Pide a un administrador que lo configure.
          </CardContent>
        </Card>
      </div>
    );
  ...
  // Negocio actual + hotel vinculado
  const business = await prisma.business.findUnique({
    ...
    select: {
      ...
      linkedHotel: { select: { id: true, name: true } },
    },
  });
  ...
  // Comensales próximos (si hay hotel vinculado)
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-2 shrink-0">
          <Button variant="outline" size="sm" asChild>
            <Link href="/app/restaurant/pos">
              <ShoppingCart className="w-4 h-4 mr-1.5" /> POS
            </Link>
          </Button>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-l-4 border-l-blue-500 py-0">
          ...
          <CardContent className="px-4 pb-4">
            ...
            <p className="text-xs text-muted-foreground">
              {activeOrders.length} órdenes abiertas
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-2 h-2 rounded-full ${
                              order.status === "OPEN" ? "bg-blue-400" : "bg-amber-400"
                            }`}
                          />
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-xs font-medium ${
                              isLong ? "text-red-500" : "text-muted-foreground"
                            }`}
                          >
                            {mins} min
                          </span>
          ...
          {/* Servicio de alimentos (solo si hay hotel vinculado) */}
          {business.linkedHotelBusinessId && business.linkedHotel && (
            <FoodServicePanel
              hotelName={business.linkedHotel.name}
              days={foodServiceDays}
            />
          )}
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {pendingLargeWithdrawals > 0 && (
                <AlertRow
                  tone="orange"
                  text={`${pendingLargeWithdrawals} retiro(s) pendiente(s)`}
              ...
              {pendingTasks > 0 && (
                <AlertRow
                  tone="amber"
                  text={`${pendingTasks} tarea(s) pendiente(s)`}
          ...
          {lowStockItems.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  <Package className="w-4 h-4 text-red-500" /> Stock crítico
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y">
                  {lowStockItems.slice(0, 5).map((item) => (
                    ...
                    >
                      <p className="font-medium truncate text-xs flex-1 min-w-0">
                        {item.name}
                      </p>
                ...
                <div className="p-2 border-t">
                  <Button variant="outline" size="sm" className="w-full text-xs" asChild>
                    <Link href={`/app/inventory/requisitions/new?businessId=${business.id}`}>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-2">
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
                <Link href="/app/restaurant/kds">
                  <UtensilsCrossed className="w-3.5 h-3.5 mr-1.5" /> KDS
                </Link>
              </Button>
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
                <Link href="/app/restaurant/menu">
                  <TrendingUp className="w-3.5 h-3.5 mr-1.5" /> Menú
                </Link>
              </Button>
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
                <Link href={`/app/inventory/requisitions/new?businessId=${business.id}`}>
              ...
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
                <Link href={`/app/manager/schedule?businessId=${business.id}`}>
                  <Calendar className="w-3.5 h-3.5 mr-1.5" /> Horarios
                </Link>
              </Button>
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
              ...
              <Button variant="outline" size="sm" className="h-10 text-xs" asChild>
                <Link href="/app/ops/kanban/activities">
                  <CheckSquare className="w-3.5 h-3.5 mr-1.5" /> Tareas
                </Link>
              </Button>
            </CardContent>
          </Card>
...
function AlertRow({
  ...
  return (
    <div
      className={`flex items-center gap-2 p-2.5 border rounded-lg text-xs ${toneClasses[tone]}`}
    >
