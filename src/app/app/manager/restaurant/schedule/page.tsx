...
export default async function SchedulePage({
  ...
  if (!scope.businessIds.includes(selectedBusinessId)) {
    redirect(`/app/manager/restaurant/schedule?businessId=${scope.businesses[0].id}`);
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
      <Button variant="ghost" size="sm" asChild>
        <Link href="/app/manager/restaurant">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
        <div className="flex flex-wrap gap-1.5">
          {scope.businesses.map((b) => (
            <Link key={b.id} href={`/app/manager/restaurant/schedule?businessId=${b.id}&week=${weekStart}`}>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" asChild>
            <Link href={`/app/manager/restaurant/schedule?businessId=${selectedBusinessId}&week=${prevWeek}`}>
          ...
          <Button variant="outline" size="sm" asChild>
            <Link href={`/app/manager/restaurant/schedule?businessId=${selectedBusinessId}&week=${nextWeek}`}>
              <ChevronRight className="w-4 h-4" />
            </Link>
          </Button>
          {weekStart !== currentWeek && (
            <Button variant="ghost" size="sm" asChild>
              <Link href={`/app/manager/restaurant/schedule?businessId=${selectedBusinessId}&week=${currentWeek}`}>
                Semana actual
              </Link>
            </Button>
          )}
