...
export default async function ManagerRanchInventoryPage({
  searchParams,
}: {
  ...
  const newHref = `/app/manager/ranch/requisitions/new${
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
      <ManagerSectionHeader
        title="Inventario"
        subtitle="Stock de productos del rancho"
        icon={<Package className="w-7 h-7 text-blue-500" />}
        businesses={scope.businesses}
        selectedBusinessId={selectedBusinessId}
        sectionBaseHref="/app/manager/ranch/inventory"
      />
      <InventoryList
        items={items}
